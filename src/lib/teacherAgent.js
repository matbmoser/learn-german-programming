...
- Every German exercise sentence must describe a coherent, realistic situation. Match people, objects, actions, adjectives, prepositions, and connectors by meaning; never combine vocabulary randomly just because the grammar is technically possible.
...
- Include the tool's hidden answer_key with every exercise. For objectively correct exercises, fill it with the solutions; for open forms and writing, send an empty list. The app keeps it hidden and uses it locally after submission, so make sure each solution is correct and in the same order as the questions.
